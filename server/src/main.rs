...
use axum::Router;
use hotm_server::{db::AppState, job_queue, routes, websocket};
...
async fn main() -> anyhow::Result<()> {
    ...
    let job_manager = Arc::new(job_queue::JobQueueManager::new(
        state.pool.clone(),
        state_arc.clone(),
    ));
    ...
    let app = Router::new()
        ...
        .route(
            "/api/v1/notes/:id/labels",
            get(routes::notes::get_metadata_labels),
        )
        .route(
            "/api/v1/notes/:id/labels",
            post(routes::notes::add_metadata_label),
        )
        .route(
            "/api/v1/notes/:id/labels/:label_id",
            delete(routes::notes::remove_metadata_label),
        )
        .route("/api/v1/labels", get(routes::notes::get_all_labels))
        .route(
            "/api/v1/notes/:id/link",
            post(routes::notes::create_note_link),
        )
        .route("/api/v1/search", get(routes::search::search))
        .route(
            "/api/v1/search/context",
            post(routes::search::generate_search_context),
        )
        .route("/api/v1/semantic", post(routes::search::semantic))
        .route(
            "/api/v1/notes/:id/related",
            get(routes::search::find_related_notes),
        )
        .route("/api/v1/tags", post(routes::taxonomy::create_tag))
        .route(
            "/api/v1/notes/:id/tags",
            put(routes::taxonomy::put_note_tags),
        )
        .route(
            "/api/v1/collections",
            post(routes::taxonomy::create_collection),
        )
        .route(
            "/api/v1/notes/:id/collection",
            put(routes::taxonomy::put_note_collection),
        )
        .route("/api/v1/notes/:id/links", post(routes::links::post_link))
        .route(
            "/api/v1/notes/:id/provenance",
            get(routes::provenance::get_provenance),
        )
        .route(
            "/api/v1/debug/revisions",
            get(routes::debug::debug_revisions),
        )
