...
fn calculate_metadata_overlap(
    ...
    if let (Some(source_entities), Some(target_entities)) =
        (source_meta.get("entities"), target_meta.get("entities"))
    {
...
pub async fn search(
    ...
    match mode {
        "fts" => {
            let hits = db::search_fts_filtered(&state, &params.q, filters, 25)
                .await
                .map_err(|e| {
        ...
        "vector" | "semantic" => {
            let vecs = match crate::ollama::embed_texts(vec![params.q.clone()], &state.embed_model)
                .await
            {
                Ok(v) => v,
                Err(e) => {
            ...
            let hits = db::search_vector_filtered(&state, query_vec, filters, 25)
                .await
        ...
        _ => {
            ...
            let fts_hits = db::search_fts_filtered(&state, &params.q, filters, 50)
                .await
                .map_err(|e| {
            ...
            let vec_hits = match crate::ollama::embed_texts(
                vec![params.q.clone()],
                &state.embed_model,
            )
            .await
            {
                Ok(vecs) => {
                    let query_vec = vecs
                        ...
                        .next()
                        .unwrap_or_else(|| vec![0.0_f32; 768]);
                    db::search_vector_filtered(&state, query_vec, filters, 50)
                        .await
                        .unwrap_or_else(|e| {
...
pub async fn semantic(
...
) -> Result<Json<SemanticResponse>, axum::http::StatusCode> {
    ...
    let query_vec = vecs
        ...
        .next()
        .unwrap_or_else(|| vec![0.0_f32; 768]);
    let notes = crate::db::search_vector_filtered(&state, query_vec, None, 25)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
Provide a brief summary (2-3 sentences) explaining how this note fits into the broader context of the user's knowledge base. Focus on themes, connections, and patterns."#,
        ...
        (crate::ollama::generate(&state.gen_model, &prompt).await).ok()
    } else {
        None
    };
...
pub fn rrf_fuse(a: Vec<SearchHit>, b: Vec<SearchHit>, limit: usize) -> Vec<SearchHit> {
    ...
    for (i, hit) in a.iter().enumerate() {
        rank_a.insert(hit.note_id, i + 1);
    }
    for (i, hit) in b.iter().enumerate() {
        rank_b.insert(hit.note_id, i + 1);
    }
    let mut scores: HashMap<Uuid, f64> = HashMap::new();
    let k = 60.0;
    for (id, r) in &rank_a {
        *scores.entry(*id).or_insert(0.0) += 1.0 / (k + *r as f64);
    }
    for (id, r) in &rank_b {
        *scores.entry(*id).or_insert(0.0) += 1.0 / (k + *r as f64);
    }
    ...
    match crate::ollama::generate(&state.gen_model, &prompt).await {
        ...
        Err(e) => {
            ...
            Ok(Json(GenerateContextResponse {
                context: format!(
                    "Found {} notes related to '{}'. Click to explore the results.",
                    req.hits.len(),
                    req.query
                ),
            }))
