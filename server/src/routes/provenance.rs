...
pub async fn get_provenance(State(state): State<AppState>, Path(id): Path<Uuid>) -> Result<Json<ProvenanceResponse>, axum::http::StatusCode> {
    ...
    let edges: Vec<ProvenanceEdge> = edges.into_iter().map(|e| ProvenanceEdge{ id: e.id, revision_id: e.revision_id, source_note_id: e.source_note_id, source_url: e.source_url, relation: e.relation, created_at_utc: e.created_at_utc }).collect();
