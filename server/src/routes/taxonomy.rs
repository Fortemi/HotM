...
) -> Result<Json<CreateTagResponse>, axum::http::StatusCode> {
    sqlx::query!(
...
pub async fn put_note_tags(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(req): Json<PutNoteTagsRequest>,
) -> Result<Json<PutNoteTagsResponse>, axum::http::StatusCode> {
    let mut tx = state
        .pool
        .begin()
        .await
    ...
    if let Some(remove) = req.remove.as_ref() {
        for t in remove {
            sqlx::query!(
                "DELETE FROM note_tag WHERE note_id = $1 AND tag_name = $2",
                id,
                t
            )
            .execute(&mut *tx)
    ...
    tx.commit()
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    let tags = sqlx::query!("SELECT tag_name FROM note_tag WHERE note_id = $1", id)
...
) -> Result<Json<CreateCollectionResponse>, axum::http::StatusCode> {
    let id = Uuid::new_v4();
    sqlx::query!(
        "INSERT INTO collection (id, name, description) VALUES ($1, $2, $3)",
        id,
        req.name,
        req.description
    )
...
pub async fn put_note_collection(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(req): Json<PutNoteCollectionRequest>,
) -> Result<Json<PutNoteCollectionResponse>, axum::http::StatusCode> {
    sqlx::query!(
    ...
    .await
    .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(PutNoteCollectionResponse {
