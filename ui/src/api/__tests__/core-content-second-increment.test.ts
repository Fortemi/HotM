...
describe('core content second increment evidence', () => {
  ...
  const response = (path: string, method: ClientMethod): unknown => {
    ...
    if (path === '/graph/snn/recompute') return { total_edges: 10, updated: 8, pruned: 2, k_used: 5, threshold_used: 0.1, dry_run: true, snn_score_distribution: [] };
  ...
  function cases(): EvidenceCase[] {
    return [
      ...
      { operationId: 'recompute_snn_scores', method: 'POST', contractPath: '/api/v1/graph/snn/recompute', clientMethod: 'post', expectedArgs: ['/graph/snn/recompute', { dry_run: true }], invoke: () => concepts.recomputeKnowledgeGraphSnn({ dry_run: true }) },
