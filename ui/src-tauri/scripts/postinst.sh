...
# We Depends: postgresql-18, so this should always pass — but check explicitly
# so a misconfigured install (e.g. someone forced dpkg -i on PG <18) gets a
# clear error instead of a cryptic matric-api migration failure later.
if su - postgres -c "psql -d matric -tAc 'SELECT uuidv7()'" >/dev/null 2>&1; then
  log "uuidv7() built-in available"
else
  warn "uuidv7() not available — Postgres 18 is required (got: $(su - postgres -c 'psql -tAc \"SHOW server_version\"' 2>/dev/null))"
  warn "matric-api migrations will fail. Install postgresql-18 from PGDG: https://wiki.postgresql.org/wiki/Apt"
fi
