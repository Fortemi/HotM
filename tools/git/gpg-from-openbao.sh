...
CREDSTORE="${HOTM_CREDSTORE_DIR:-/etc/credstore.encrypted}"
...
[[ -n "$RUNTIME_PARENT" ]] || { echo "FAIL: a writable tmpfs is required for signing material." >&2; exit 1; }

TMP="$(mktemp -d "$RUNTIME_PARENT/hotm-gpg.XXXXXX")"
GNUPGHOME="$TMP/gnupg"
