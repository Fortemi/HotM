-- ============================================================================
-- HotM Database Schema - Consolidated Clean Schema
-- ============================================================================
...
CREATE TABLE note_revision (
  ...
  model TEXT  -- Migration 0006: Track which generation model was used
);
...
CREATE TABLE note_tag (
  ...
  source TEXT DEFAULT 'manual', -- 'ai', 'manual', 'system'
...
CREATE TABLE link (
  ...
  created_at_utc TIMESTAMPTZ NOT NULL,
  metadata JSONB DEFAULT '{}'::jsonb,  -- Migration 0004: Link metadata (keywords, etc.)
...
CREATE TABLE user_metadata_label (
  ...
  note_id UUID NOT NULL REFERENCES note(id) ON DELETE CASCADE,
  label TEXT NOT NULL,
  color TEXT, -- Optional color for UI display (hex color)
...
CREATE TABLE activity_log (
  ...
  action TEXT NOT NULL, -- 'create_note', 'revise', 'delete', etc.
...
CREATE TABLE embedding (
  ...
  text TEXT NOT NULL,
  vector vector(768), -- Assuming 768 dimensions for nomic-embed-text
  model TEXT NOT NULL,
...
CREATE TABLE provenance_edge (
  ...
  source_url TEXT,
  relation TEXT NOT NULL, -- 'derived_from', 'references', 'contradicts', etc.
...
CREATE TABLE job_queue (
  ...
  progress_message TEXT,  -- Migration 0002: WebSocket updates
  logs TEXT[],  -- Migration 0003: Job logs
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX idx_link_metadata ON link USING GIN (metadata);  -- Migration 0004: Link metadata
...
CREATE INDEX idx_job_queue_completed_at ON job_queue(completed_at DESC)   -- Migration 0003: Job history queries
...
BEGIN
    ...
    IF avg_duration IS NULL THEN
        CASE p_job_type
            WHEN 'ai_revision' THEN avg_duration := 15000; -- 15 seconds
            WHEN 'embedding' THEN avg_duration := 5000;    -- 5 seconds
            WHEN 'linking' THEN avg_duration := 3000;       -- 3 seconds
            WHEN 'context_update' THEN avg_duration := 10000; -- 10 seconds
            WHEN 'title_generation' THEN avg_duration := 5000; -- 5 seconds
            ELSE avg_duration := 8000; -- 8 seconds default
        END CASE;
    END IF;
...
-- Function to refresh the tags view
...
-- Migration 0003: Function to cleanup old jobs (keep last 100)
...
-- Materialized view for all unique tags
CREATE MATERIALIZED VIEW all_tags_view AS
SELECT DISTINCT
    tag,
    'ai' as source,
...
CREATE UNIQUE INDEX idx_all_tags_view_tag_source ON all_tags_view (tag, source);
CREATE INDEX idx_all_tags_view_tag ON all_tags_view (tag);
