...
### 🛠 `__HOTM_HOST__` adapter — legacy `__BT6_HOST__` compatibility (#192)

Third-party shells that embed HotM using the older `__BT6_HOST__` injection-point name now work unmodified. The runtime accepts both injection points and forwards the `version` field through to the host adapter so embedders can detect protocol revision. This is a fix for embedders who picked up HotM during the BT6→HotM rename and pinned the old name.
...
| Embedding shells using `__BT6_HOST__` | Now compatible (was: required rename) |

## Commit references

- `cdcdfd1` — `chore(aiwg): refresh framework to 2026.4.0-rc.31, drop codex provider`
- `a1feca4` — `fix(host-adapter): accept legacy __BT6_HOST__ + version field + docs (#192)`
