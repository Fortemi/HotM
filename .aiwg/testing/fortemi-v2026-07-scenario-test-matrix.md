...
| UX-FORTEMI-005 Backup, TUS, and Portable Shard Parity | #257 | API unit, service unit, Backup component, copy assertions | `ui/src/api/__tests__/backup.test.ts`, `ui/src/api/__tests__/attachments.test.ts`, `ui/src/services/__tests__/tusUploader.test.ts`, `ui/src/services/__tests__/uploadStore.test.ts`, `ui/src/components/__tests__/JobQueueMonitor.test.tsx`, `ui/src/components/backup/__tests__/BackupManager.test.tsx` | Backup parity covers legacy backup, shard base64/multipart import, database download/snapshot/upload/restore, memory download, archive download/upload, list/detail/swap, metadata get/update, BackupManager route-group controls, and portable sidecar limitation copy. TUS parity covers POST creation metadata, server `media_optimize` query handling, OPTIONS discovery, HEAD resume headers, PATCH required headers/offset, DELETE termination, GET finalization, termination aborts, offset mismatch, chunk-too-large, expired/not-found recovery, and no checksum-extension claim. | OPTIONS, HEAD, GET, PATCH, DELETE, offset mismatch, chunk-too-large, expired/not-found recovery, termination, resume, no checksum-extension claim, byte-sidecar not-restored copy, metadata route payloads. | Implemented |
