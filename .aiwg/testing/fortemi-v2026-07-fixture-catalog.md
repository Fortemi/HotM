...
| `knowledge-shard-full-v1` | #272 | `knowledgeShard.test.ts`, `backup.test.ts`, `BackupManager` tests, pinned Fortemi runtime/paired receipts | `exact-2.0.0`, `all-33-components`, `all-34-count-fields`, `direct-stream`, `signed-dry-run`, `tampered-zero-mutation`, `missing-zero-mutation`, `oversized-zero-mutation`, `unsupported-zero-mutation`, `skewed-zero-mutation` |
