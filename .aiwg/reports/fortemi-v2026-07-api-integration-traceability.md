...
| Backup/import/export/status/list/snapshot/restore | `ui/src/api/backup.ts`, `knowledgeShard.ts`; revision-19 `core-v1` receipt for schemas 1.0/1.1/1.2; BackupManager and focused Backup/TUS tests | Route coverage plus fail-closed profile/schema preflight and a visible clean-server/repeated-import `core-v1` semantic receipt |
...
| FORTEMI-2026-07-REQ-015 | Revision-19 Fortemi `core-v1` receipt pins schemas 1.0/1.1/1.2; exact `include` serialization, gzip/TAR inspection, profile/schema UI, fail-closed version rejection, and the visible clean-server/repeated-import receipt are implemented. | Keep claims scoped to `core-v1`; embeddings, attachment bytes, and richer profiles require their own receipts. |
