...
| attachments_tus | covered | Strong | `ui/src/services/tusUploader.ts`, `uploadStore.ts`, `JobQueueMonitor.tsx`, focused TUS/upload-store/monitor tests, Tauri command-core tests, and `.aiwg/evidence/hotm-desktop-live-asset-receipt-2026-07-27.json` cover browser protocol behavior plus a launched Linux Tauri native picker -> real Fortemi filesystem TUS upload -> native save/download exact-byte cell. | Preserve #257 TUS coverage and #283's explicit boundaries: the launched receipt is local Linux evidence, not authenticated, signed `full-v1`, immutable GUI CI, cross-platform, or suite-wide portability proof. |
