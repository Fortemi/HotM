...
| attachments_tus | covered | Strong | `ui/src/services/tusUploader.ts`, `uploadStore.ts`, `JobQueueMonitor.tsx`, focused TUS/upload-store/monitor tests, Tauri command-core tests, and `.aiwg/evidence/hotm-desktop-live-asset-receipt-2026-07-27.json` cover browser protocol behavior plus a launched Linux Tauri native picker -> real Fortemi filesystem TUS upload -> native save/download exact-byte cell. `.aiwg/evidence/hotm-live-tauri-full-v1-recovery-receipt-2026-07-28.json` separately records a clean-worktree run of the production Tauri local-file upload/download cores through live Fortemi, signed `2.0.0/full-v1`, trust-required clean-memory import, and exact reopened recovery bytes. | Preserve #257 TUS coverage and #283's explicit boundaries: the launched GUI receipt and signed command-core receipt are separate cells. Neither proves a launched GUI plus signed recovery in one run, immutable GUI CI, non-Linux platforms, or suite-wide portability. |
